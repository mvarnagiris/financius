...
public class LockActivity extends BaseActivity implements LockView.OnPasswordEnteredListener {
    ...
    public static void start(Context context, SecurityType securitySecurityType) {
        ActivityStarter.begin(context, LockActivity.class).extra(EXTRA_SECURITY_TYPE, securitySecurityType).start();
    }
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        securityType = (SecurityType) getIntent().getExtras().getSerializable(EXTRA_SECURITY_TYPE);
    ...
    private void startNewLockProcess(LockView.State restoredState) {
        switch (securityType) {
            ...
            case Pin:
                ...
                break;
            default:
                throw new IllegalStateException("Security type " + securityType + " is not supported.");
        }
    ...
    private void onNewLockCreated(String password) {
        ...
        getSecurity().setType(securityType, password);
        getSecurity().notifyChanged();
        endpointsApi.syncConfig();
        finish();
    }
