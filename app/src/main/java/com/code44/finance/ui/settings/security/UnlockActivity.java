...
public class UnlockActivity extends BaseActivity implements LockView.OnPasswordEnteredListener {
    ...
    public static void startForResult(Activity activity, int requestCode, boolean closeIfNotVerified) {
        ActivityStarter.begin(activity, UnlockActivity.class)
                .extra(EXTRA_CLOSE_IF_NOT_VERIFIED, closeIfNotVerified)
    ...
    private void startUnlockProcess() {
        switch (getSecurity().getSecurityType()) {
            case None:
                onVerificationSuccessful();
                return;
            ...
            case Pin:
                ...
                break;
            default:
                throw new IllegalStateException("Security type " + getSecurity().getSecurityType() + " is not supported.");
        }
