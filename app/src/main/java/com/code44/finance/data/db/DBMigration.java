...
public final class DBMigration {
    ...
    public static void upgradeV23(SQLiteDatabase db, CurrenciesManager currenciesManager) {
        try {
            ...
            try {
                ...
                Cursor cursor = Tables.CurrencyFormats.getQuery()
                        .projection("currencies_is_default")
                        .from(db, Tables.CurrencyFormats.TABLE_NAME)
                        .execute();
                ...
                cursor = Tables.Accounts.getQuery()
                        .clearSelection()
                        .clearArgs()
                        .clearSort()
                        .clearSort()
                        .projection("accounts_currency_id")
                        .from(db, Tables.Accounts.TABLE_NAME)
                        .execute();
                if (cursor != null && cursor.moveToFirst()) {
                    ...
                    do {
                        ...
                        db.update(Tables.Accounts.TABLE_NAME, values, Tables.Accounts.LOCAL_ID + "=?", new String[]{
                                String.valueOf(cursor.getLong(iLocalId))});
                    } while (cursor.moveToNext());
                }
                IOUtils.closeQuietly(cursor);
    ...
    public static void upgradeV24(SQLiteDatabase db) {
        try {
            ...
            values.clear();
            values.put(Tables.CurrencyFormats.SYNC_STATE.getName(), SyncState.None.asInt());
            db.update(Tables.CurrencyFormats.TABLE_NAME, values, null, null);

            values.clear();
            values.put(Tables.Categories.SYNC_STATE.getName(), SyncState.None.asInt());
            db.update(Tables.Categories.TABLE_NAME, values, null, null);

            values.clear();
            values.put(Tables.Tags.SYNC_STATE.getName(), SyncState.None.asInt());
            db.update(Tables.Tags.TABLE_NAME, values, null, null);

            values.clear();
            values.put(Tables.Accounts.SYNC_STATE.getName(), SyncState.None.asInt());
            db.update(Tables.Accounts.TABLE_NAME, values, null, null);

            values.clear();
            values.put(Tables.Transactions.SYNC_STATE.getName(), SyncState.None.asInt());
    ...
    public static void fixTransactionsWithNotExistingAccounts(SQLiteDatabase db) {
        final String tables = Tables.Transactions.TABLE_NAME + " left join " + Tables.Accounts.TABLE_NAME + " as " + Tables.Accounts.TEMP_TABLE_NAME_FROM_ACCOUNT + " on " + Tables.Accounts.ID
                .getNameWithTable(Tables.Accounts.TEMP_TABLE_NAME_FROM_ACCOUNT) + "=" + Tables.Transactions.ACCOUNT_FROM_ID;
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                values.put(Tables.Transactions.STATE.getName(), TransactionState.Pending.asInt());
                db.update(Tables.Transactions.TABLE_NAME, values, Tables.Transactions.LOCAL_ID + "=?", new String[]{
    ...
    private static String v19MigrateCurrencies(SQLiteDatabase db) {
        ...
        final String[] projection = {oldTableName + "_server_id", oldTableName + "_code", oldTableName + "_symbol",
                                     oldTableName + "_decimals", oldTableName + "_decimal_separator", oldTableName + "_group_separator",
                                     oldTableName + "_symbol_format", oldTableName + "_is_default", oldTableName + "_exchange_rate"};
        ...
        if (cursor != null && cursor.moveToFirst()) {
            ...
            do {
                ...
                final SymbolPosition symbolPosition = "LF".equals(symbolPositionOld) ? SymbolPosition.FarLeft : "LC".equals(symbolPositionOld) ? SymbolPosition.CloseLeft : "RC"
                        .equals(symbolPositionOld) ? SymbolPosition.CloseRight : SymbolPosition.FarRight;
    ...
    private static String v19MigrateAccounts(SQLiteDatabase db, String tempCurrenciesTable) {
        ...
        final String[] projection = {oldTableName + "_server_id", "currencies_server_id", oldTableName + "_title", oldTableName + "_note",
                                     oldTableName + "_show_in_totals"};
        ...
        if (cursor != null && cursor.moveToFirst()) {
            ...
            do {
                ...
                account.prepareForContentValues();

                final ContentValues values = new ContentValues();
                values.put(Tables.Accounts.ID.getName(), account.getId());
                values.put(Tables.Accounts.MODEL_STATE.getName(), account.getModelState().asInt());
                values.put(Tables.Accounts.SYNC_STATE.getName(), account.getSyncState().asInt());
                values.put("accounts_currency_id", currencyFormat.getId());
                values.put(Tables.Accounts.TITLE.getName(), account.getTitle());
                values.put(Tables.Accounts.NOTE.getName(), account.getNote());
                values.put(Tables.Accounts.BALANCE.getName(), account.getBalance());
                values.put(Tables.Accounts.INCLUDE_IN_TOTALS.getName(), account.includeInTotals());
    ...
    private static String v19MigrateCategories(SQLiteDatabase db) {
        ...
        final String[] projection = {oldTableName + "_server_id", oldTableName + "_title", oldTableName + "_color", oldTableName + "_type",
                                     oldTableName + "_order", oldTableName + "_level"};
        final String selection = oldTableName + "_delete_state = 0 and " + oldTableName + "_origin = 1";
        final Cursor cursor = db.query(tempTableName, projection, selection, null, null, null, null);
        if (cursor != null && cursor.moveToFirst()) {
    ...
    private static String v19MigrateTransactions(SQLiteDatabase db, String tempAccountsTable, String tempCategoriesTable) {
        ...
        final String[] projection = {oldTableName + "_server_id", oldTableName + "_date", oldTableName + "_amount",
                                     oldTableName + "_exchange_rate", oldTableName + "_note", oldTableName + "_state",
                                     oldTableName + "_show_in_totals", tempFromAccountsTable + ".accounts_server_id",
                                     tempToAccountsTable + ".accounts_server_id", tempChildCategoriesTable + "._id",
                                     tempChildCategoriesTable + ".categories_server_id", tempChildCategoriesTable + ".categories_level",
                                     tempChildCategoriesTable + ".categories_type", tempParentCategoriesTable + ".categories_server_id"};
        final String selection = oldTableName + "_delete_state = 0";
    ...
    private static void v20FixCategoriesIds(SQLiteDatabase db) {
        final Cursor cursor = Query.create().projection(Tables.Categories.ID.getName()).from(db, Tables.Categories.TABLE_NAME).execute();
